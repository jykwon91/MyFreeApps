"""__APP_DISPLAY_NAME__ encryption helpers.

Two key families derived from a single ``encryption_key`` master via HKDF:

1. **PII** — ``salt=b"__APP_SLUG__-v1"``,
   ``info=b"__APP_SLUG__-pii-encryption"``. Used by the
...
_PII_SALT = b"__APP_SLUG__-v1"
_PII_INFO = b"__APP_SLUG__-pii-encryption"
