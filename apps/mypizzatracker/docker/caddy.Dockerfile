...
#   python -m platform_shared.infra.render --app mypizzatracker
#
...
# All COPY paths are relative to MyFreeApps/, not apps/mypizzatracker/.
#
...
COPY apps/mypizzatracker/frontend/package.json ./apps/mypizzatracker/frontend/
RUN npm ci --ignore-scripts
COPY packages/shared-frontend ./packages/shared-frontend
COPY apps/mypizzatracker/frontend ./apps/mypizzatracker/frontend
...
RUN npm run build --workspace=apps/mypizzatracker/frontend

FROM caddy:2-alpine
COPY --from=frontend-build /repo/apps/mypizzatracker/frontend/dist /srv/frontend
COPY apps/mypizzatracker/docker/Caddyfile.docker /etc/caddy/Caddyfile
