...
GET    /users/me/export -- app-specific data export.
...
async def export_user_data(
    ...
    """Export all user data as JSON. Populate domain rows in account_service.build_export."""
    payload = await account_service.build_export(db, user)
    timestamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    return JSONResponse(
        content=payload,
        headers={
            "Content-Disposition": f"attachment; filename=mypizzatracker-export-{timestamp}.json",
        },
    )
