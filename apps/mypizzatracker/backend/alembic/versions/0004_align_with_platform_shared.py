"""Align with platform_shared models -- table names, schemas, enums
...
Drift fixed (same set MGA's 0004 addressed):
  ...
  3. ``user.role`` switched from ``String(20) + CheckConstraint`` to the
     ...
     Without this, fastapi-users INSERTs fail with
     ``type "user_role" does not exist`` at the first auth attempt.

MyPizzaTracker does not have the MGA-specific game / map / map_zone /
site / utility_type taxonomy tables, so the timestamp-add portion of
MGA's 0004 is skipped here. MPT's own domain tables (drop / slot /
pizza_type / topping_type) already carry ``created_at`` + ``updated_at``.

Drops + recreates ``audit_log`` and ``auth_event`` rather than ALTER --
the column shapes diverge too far for a clean ALTER. MPT is not yet in
production so no data-loss concern. Local dev DBs with ad-hoc patches
should be rebuilt:
