"""MyGamingAssistant encryption helpers.

Two key families derived from a single ``encryption_key`` master via HKDF:

1. **PII** — ``salt=b"mygamingassistant-v1"``,
   ``info=b"mygamingassistant-pii-encryption"``. Used by the
...
_PII_SALT = b"mygamingassistant-v1"
_PII_INFO = b"mygamingassistant-pii-encryption"
...
def encrypt_pii(value: str | None) -> str | None:
    """Encrypt PII with MGA's PII key family."""
    if value is None:
        return None
