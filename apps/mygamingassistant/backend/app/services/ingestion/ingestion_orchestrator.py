...
from app.services.ingestion.chapter_media import generate_chapter_media
...
async def _process_chapter(
    ...
    if classifier_ran and result is not None and result.is_lineup:
        ...
        # Best-effort media tail: throw clip → landing clip → micro clips →
        # STAND/LANDING posters → throw technique. Extracted to chapter_media
        # so this orchestrator stays under the file-size guard; each step there
        # is independently non-fatal and self-committing (a media failure never
        # rolls back the already-committed lineup + classifier suggestions).
        # Reuses the on-disk source video and the precomputed throw timings so
        # ingest adds no extra Claude spend beyond the grid classifier.
        await generate_chapter_media(
            db,
            lineup=lineup,
            chapter=chapter,
            source=source,
