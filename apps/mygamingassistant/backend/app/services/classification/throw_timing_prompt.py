"""Throw-timing-localization system-prompt body.

Extracted from :mod:`throw_timing_classifier` per
apps/mygamingassistant/CLAUDE.md "Tech Debt Policy" (no-growth on files
already over 500 LOC -> split a sibling module + import back). The classifier
was over the 500-LOC growth guard and ~240 of those lines were this single
prompt constant. It lives here as pure text -- no logic -- and the classifier
formats (``{n}`` -> frame count) and sends it. Mirrors the sibling
:mod:`aim_timing_prompt` split; keeping the prompt in its own module also lets
prompt-iteration diffs stand alone from code-path changes.
"""
...
- earlier_demonstration_result_index: the 1-based frame where an EARLIER
  ...
  CROSS-CHAPTER BLEED EXCLUSION (critical — operator audit 2026-05-30, lineup
  7bd971c3 "Market Window": the FIRST frame showed a dense smoke bloom at
  CATWALK — the PREVIOUS lineup's smoke still hanging in the air as this
  chapter opened — and it was wrongly reported here, pulling the throw onto the
  chapter boundary): an EARLIER demonstration of THIS lineup lands at the SAME
  target / aiming landmark as your release_index throw. A utility effect that
  is (a) at a DIFFERENT map area / landmark than where your release lands, OR
  (b) already fully present in the FIRST one or two frames of the set with NO
  throw of THIS lineup visible before it in the set, is a PREVIOUS lineup's
  lingering effect bleeding across the chapter boundary — NOT an earlier
  demonstration of this lineup. Set earlier_demonstration_result_index = null
  in that case; only report it for a genuine earlier repeat of the SAME throw
  to the SAME place. (A hard backstop also drops an earlier result reported in
  the chapter's opening seconds, but get it right here so the throw is found in
  one pass.)
