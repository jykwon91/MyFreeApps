"""Lineup lifecycle — create, read, update, hide, accept + classifier writeback.

Covers the core CRUD shape of a lineup plus the pending → accepted review
flow and the classifier-suggestions writeback that runs alongside it.
The pane-specific writers (clip, landing, micro, technique, screenshot)
live in their own sibling modules — see this package's ``__init__``.
"""
