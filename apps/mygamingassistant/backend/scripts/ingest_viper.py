"""Data-driven Viper lineup ingest driver — one JSON per map under scripts/viper-spans/.

Reusable across all 9 Valorant maps for the Viper full-build (Initiative 18).
Each scripts/viper-spans/<map>.json has:
...
- cs = floor(STAND.start), unique per lineup within the video (these B3ast sources
  have NO chapters — cs is a synthetic clip-storage key, not native chapter data).
...
Subcommands (MAIN checkout venv, cwd = backend, PG:5433 up):
  ...
  .venv/Scripts/python.exe scripts/ingest_viper.py <map> plan
  .venv/Scripts/python.exe scripts/ingest_viper.py <map> create
  .venv/Scripts/python.exe scripts/ingest_viper.py <map> recut
  .venv/Scripts/python.exe scripts/ingest_viper.py <map> accept
"""
...
GAME_SLUG = "valorant"
AGENT_HINT = "viper"
SPANS_DIR = ROOT / "scripts" / "viper-spans"


def _load(map_slug: str) -> dict:
    path = SPANS_DIR / f"{map_slug}.json"
    if not path.exists():
...
def _validate(data: dict, zones: dict, utils: dict) -> None:
    ...
    for ln in data["lineups"]:
        ...
        if ln["ability"] not in utils:
            errs.append(f"cs={ln['cs']} ability {ln['ability']!r} not in utility_type")
...
async def cmd_plan(map_slug: str) -> None:
    data = _load(map_slug)
    async with AsyncSessionLocal() as db:
        _, vmap, zones, utils = await _resolve(db, data)
        _validate(data, zones, utils)
        print(f"{data['map_slug']} — {len(data['lineups'])} lineups, video {data['video_id']}")
        for ln in data["lineups"]:
            sp = ln["spans"]
            print(f"  cs={ln['cs']:4} {ln['ability']:11} {ln['stand']:7}->{ln['target']:7} "
                  f"{ln['side']} thr={sp['throw'][0]:.2f} :: {ln['title']}")
        print("validation OK (all utilities + zones resolve).")


async def cmd_create(map_slug: str) -> None:
    data = _load(map_slug)
    vid = data["video_id"]
    ...
    async with AsyncSessionLocal() as db:
        ...
        if src_id is None:
            src = Source(kind="youtube_playlist", config_json={
                "url": url, "map_hint": data["map_slug"], "game_hint": GAME_SLUG, "agent_hint": AGENT_HINT})
...
def cmd_recut(map_slug: str) -> None:
    data = _load(map_slug)
    vid = data["video_id"]
...
async def cmd_accept(map_slug: str) -> None:
    data = _load(map_slug)
    vid = data["video_id"]
    async with AsyncSessionLocal() as db:
        game_id, vmap, zones, utils = await _resolve(db, data)
        ...
        for ln in data["lineups"]:
            ...
            print(f"  ACCEPT cs={ln['cs']:4} {ln['ability']:11} {ln['stand']}->{ln['target']} {ln['side']} :: {ln['title']}")
            ok += 1
        await db.commit()
...
def main() -> None:
    if len(sys.argv) < 3:
        raise SystemExit("usage: ingest_viper.py <map> {plan|create|recut|accept}")
    map_slug, sub = sys.argv[1], sys.argv[2]
    if sub == "recut":
        cmd_recut(map_slug)
    elif sub in ("plan", "create", "accept"):
        asyncio.run({"plan": cmd_plan, "create": cmd_create, "accept": cmd_accept}[sub](map_slug))
    else:
