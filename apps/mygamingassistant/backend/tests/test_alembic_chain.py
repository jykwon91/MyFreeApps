...
def test_single_head_is_0024(script_directory: ScriptDirectory) -> None:
    """The DAG must resolve to exactly one head and it must be 0024.
    ...
    assert heads[0] == "0024", (
        f"Expected head 0024 (wow_map_capture), "
        f"got {heads[0]}."
    )
...
def test_0017_down_revision_points_at_0016(
    script_directory: ScriptDirectory,
) -> None:
    """0017 must chain directly off 0016 (lineup.stand_ts + stand_localized_at)."""
    rev = script_directory.get_revision("0017")
    assert rev.down_revision == "0016"


def test_0018_down_revision_points_at_0017(
    script_directory: ScriptDirectory,
) -> None:
    """0018 must chain directly off 0017 (lineup.aim_ts + aim_localized_at)."""
    rev = script_directory.get_revision("0018")
    assert rev.down_revision == "0017"


def test_0019_down_revision_points_at_0018(
    script_directory: ScriptDirectory,
) -> None:
    """0019 must chain directly off 0018 (agent table + utility_type.agent_id)."""
    rev = script_directory.get_revision("0019")
    assert rev.down_revision == "0018"


def test_0020_down_revision_points_at_0019(
    script_directory: ScriptDirectory,
) -> None:
...
def test_0022_down_revision_points_at_0021(
    script_directory: ScriptDirectory,
) -> None:
...
def test_0024_down_revision_points_at_0023(
    script_directory: ScriptDirectory,
) -> None:
    """0024 must chain directly off 0023 (wow_map_capture)."""
    rev = script_directory.get_revision("0024")
