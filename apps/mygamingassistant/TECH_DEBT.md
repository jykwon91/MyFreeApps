...
### [Data] Attribution - 19 shipped lineups credit the same creator under a shortened channel name

- **Severity:** Medium
- **Location:** backend/scripts/{fade,kay-o,sova,viper}-spans/*.json `author`, and the per-agent pack builders that wrote them
- **Problem:** Every pack builder hardcodes the creator string per AGENT rather than per SOURCE. build_cypher_pack has been fixed (AGENTS[...]["sources"] maps video_id -> creator and an unregistered video aborts), but the other agents' builders still carry a single literal. The visible symptom is that one channel ships under two different names: `Tseeky` on 19 videos (kay-o ascent-2026/bind/breeze/fracture/haven/icebox/lotus/split, fade summit, sova summit, viper summit) and `Tseeky - Pro Valorant Lineups` — the channel's real name, per its own oEmbed — on the rest. Same creator, so nobody is miscredited, but the library shows them as two people and any future per-creator grouping splits in half. The severe form of this bug (crediting a different person entirely) already shipped on both Summit Cypher sources and was corrected in the same PR that introduced the source-keyed registry.
- **Recommendation:** Give every remaining pack builder the same source-keyed `sources` registry + `--author` flag + fail-loud-on-unknown-video that build_cypher_pack now has, seeded from each pack's real channel name. Then normalise the 19 packs and the matching `lineup.attribution_author` rows to `Tseeky - Pro Valorant Lineups`, re-export and re-publish. Re-run the audit afterwards: resolve every distinct `youtube_video_id` through `https://www.youtube.com/oembed?url=...&format=json` and diff `author_name` against the DB — it was 0 unresolvable and 21 mismatches before this entry.
