...
- **Partly contained:** The tables now live in `legacy_pack_zones.py` and are scoped per AGENT, not per map (`AGENTS[...]["legacy_zone_maps"]`): cypher keeps all eight, and killjoy keeps only ascent, where it shipped against them. The Haven Killjoy wave showed why. With map-wide scoping, a new agent on a legacy map silently inherited the wrong readings, and 7 of 18 rows came out as b-site (`mid courtyard`) or a-site (the bare `a` key catching "A Garden"). The eight tables still exist, so the reconcile above remains the fix.

### [RESOLVED — follow-up PR] [Data] Attribution - creator names are point-in-time snapshots and drift when a channel renames itself

- **Severity:** Medium
- **Location:** backend/scripts/refresh_attribution.py; `lineup.attribution_author`; the `author` field of every backend/scripts/*-spans/*.json
- **Problem:** `attribution_author` is captured ONCE when a pack is built — build_items.py reads yt-dlp's `uploader`, build_cypher_pack.py reads its own per-source registry — and nothing revisits it. Channels rename themselves, and a rename never reaches rows already banked, so one creator shows up as two people. That had happened: 19 videos / 526 rows carried `Tseeky` while everything ingested after the rename carried the channel's current `Tseeky - Pro Valorant Lineups`. (An earlier draft of this entry blamed per-agent literals in five builders. That was wrong — only one pack builder exists, and the others read the uploader from the chapters dump. The cause is the snapshot going stale, not a hardcoded name.)
- **Recommendation:** DONE. `refresh_attribution.py` re-resolves every distinct `youtube_video_id` through the channel's own oEmbed record, reports drift, and with `--apply` rewrites both the DB rows and the packs; re-export afterwards. It is re-runnable by design, because a one-off UPDATE fixes today's drift and guarantees tomorrow's — run it after any batch. Note it cannot distinguish a rename from a miscredit (the severe form, which shipped once on both Summit Cypher sources), so read the pairings it prints before applying.
