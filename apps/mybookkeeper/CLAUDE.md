...
- Account-level login lockout: 5 consecutive bad-password attempts trigger an exponential lock (1 min → 5 min → 15 min → 1 hour → 24 hours). Counter auto-resets if >24 h have elapsed since the last failure. Lock state stored in `users.failed_login_count`, `users.locked_until`, `users.last_failed_login_at`. Two enforcement layers: (1) `check_account_not_locked` FastAPI dependency rejects at the route level (HTTP 429) before the password is checked; (2) `UserManager.authenticate` increments the counter and applies the lock on actual password failures. Never reveal the exact lock-expiry time to callers — return a generic "Too many attempts" message. Constants: `LOCKOUT_THRESHOLD=5`, `LOCKOUT_AUTORESET_HOURS=24` in `core/config.py`. Pure policy logic (`record_failed_login`, `record_successful_login_update`, `is_locked`, `lock_duration_for`, `autoreset_update_if_stale`, `emit_locked_login_event`) lives in `platform_shared.services.account_lockout` (PR M7 — final M-series PR). MBK's `app/core/auth.py` injects the configured threshold + autoreset window when calling the shared helpers; `app/services/user/account_service.py` keeps thin back-compat shims so existing imports of `record_failed_login` / `record_successful_login` / `is_locked` from there still resolve.
- Per-IP login rate limit (10 req/5min on `/auth/jwt/login` and `/auth/totp/login`) returns the same generic 429 body as account lockout — callers cannot distinguish which gate fired. `LOGIN_BLOCKED_RATE_LIMIT` audit event written on every per-IP block (metadata: `email_domain`, `ip`). Shared body string lives in `app/core/auth_messages.py::RATE_LIMIT_GENERIC_DETAIL`.
...
- Production routing is two-tier:
  - Host Caddy (`infra/Caddyfile` at the monorepo root, lives at `/etc/caddy/Caddyfile` on VPS — SHARED across all apps so a deploy of one app does not wipe another's entry) is intentionally a thin TLS terminator + reverse-proxy to docker Caddy on `127.0.0.1:8094`. Only HSTS + nosniff + Referrer-Policy at this layer.
...
- Deploy workflow auto-syncs `infra/Caddyfile` (shared) → `/etc/caddy/Caddyfile` on the VPS and runs a bundle freshness tripwire (extracts script src from live index.html, asserts the file exists in caddy container).
...
Last verified auto-deploy from MyFreeApps main: 2026-04-26 (post-#84 ssh-action SHA fix + SSH_HOST/SSH_USER/SSH_PRIVATE_KEY secrets configured + private key re-uploaded directly + /srv/myfreeapps switched from feat/migrate-mybookkeeper to main).
