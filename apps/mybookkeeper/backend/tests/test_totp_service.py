...
class TestValidateTotpForLogin:
    ...
    async def test_valid_totp_code_returns_true(self, db: AsyncSession) -> None:
        ...
        code = pyotp.TOTP(secret).now()
        result = await totp_service.validate_totp_for_login("login@example.com", code)
        assert result is True
    ...
    async def test_wrong_totp_code_returns_false(self, db: AsyncSession) -> None:
        ...
        result = await totp_service.validate_totp_for_login("wrongcode@example.com", "000000")
        assert result is False
    ...
    async def test_valid_recovery_code_returns_true(self, db: AsyncSession) -> None:
        ...
        result = await totp_service.validate_totp_for_login("recovery@example.com", "AABBCCDD")
        assert result is True
    ...
    async def test_recovery_code_consumed_after_use(self, db: AsyncSession) -> None:
        ...
        result = await totp_service.validate_totp_for_login("consumed@example.com", "AABBCCDD")
        assert result is False
    ...
    async def test_invalid_recovery_code_returns_false(self, db: AsyncSession) -> None:
        ...
        result = await totp_service.validate_totp_for_login("badrec@example.com", "XXXXXXXX")
        assert result is False
    ...
    async def test_totp_not_enabled_returns_false(self, db: AsyncSession) -> None:
        ...
        code = pyotp.TOTP(secret).now()
        result = await totp_service.validate_totp_for_login("notenabled@example.com", code)
        assert result is False

    @pytest.mark.asyncio
    async def test_unknown_email_returns_false(self, db: AsyncSession) -> None:
        result = await totp_service.validate_totp_for_login("ghost@example.com", "123456")
        assert result is False
