...
async def org(db: AsyncSession, owner: User) -> Organization:
    ...
    org = await organization_repo.create(db, "Res Test Org", owner.id)
    await db.commit()
...
def _patch_service_session(db: AsyncSession):
    ...
    with patch("app.services.transactions.reservation_query_service.AsyncSessionLocal", _fake_session):
        yield
...
class TestListReservations:
    ...
    async def test_returns_200(
        ...
        await _seed_reservation(db, org, prop)
        resp = await client.get("/reservations")
    ...
    async def test_filters_by_property(
        ...
        await _seed_reservation(db, org, prop)
        resp = await client.get("/reservations", params={"property_id": str(prop.id)})
        assert resp.status_code == 200
        data = resp.json()
...
class TestOccupancyStats:
    ...
    async def test_returns_stats(
        ...
        resp = await client.get("/reservations/occupancy", params={
