...
async def execute_readonly_query(
    ...
    result = await db.execute(text(limited))
...
async def bulk_update_property(
    ...
    result = await db.execute(
        ...
        """),
        {"prop_id": target_property_id, "vendor": f"%{vendor}%", "pattern": f"%{filename_pattern}%"},
    )
...
async def bulk_update_sub_category(
    ...
    result = await db.execute(
        ...
        """),
        {"sub_cat": new_sub_category, "vendor": f"%{vendor}%", "pattern": f"%{description_pattern}%"},
    )
    return result.rowcount
...
async def bulk_soft_delete(
    ...
    conditions = ["t.vendor ILIKE :vendor", "t.deleted_at IS NULL"]
    params: dict[str, str] = {"vendor": f"%{vendor}%"}
