...
async def _compute_schedule_se(db: AsyncSession, tax_return: TaxReturn) -> int:
    ...
    wage_base = SE_TAX_WAGE_BASE.get(tax_return.tax_year)
    if wage_base is None:
        latest_year = max(SE_TAX_WAGE_BASE)
        wage_base = SE_TAX_WAGE_BASE[latest_year]
        logger.warning(
            "SE tax wage base not configured for %d; using %d value ($%s)",
            tax_return.tax_year, latest_year, wage_base,
        )

    if net_earnings <= wage_base:
        se_tax = (net_earnings * SE_TAX_RATE_FULL).quantize(Decimal("0.01"))
    else:
        se_tax_base = (wage_base * SE_TAX_RATE_FULL).quantize(Decimal("0.01"))
        se_tax_excess = ((net_earnings - wage_base) * SE_TAX_RATE_MEDICARE).quantize(Decimal("0.01"))
...
async def _compute_1040_aggregation(db: AsyncSession, tax_return: TaxReturn) -> int:
    ...
    # Schedule SE deductible_half -> Schedule 1 Line 15 (reduces AGI)
    se_deductible_half = field_map.get("schedule_se", {}).get(
        "deductible_half", Decimal("0"),
    )
    if se_deductible_half:
        inst_sched_1 = await tax_return_repo.upsert_form_instance(
            db, tax_return.id, "schedule_1", "computed",
            instance_label="Schedule 1",
        )
        forms_updated += 1
        await tax_return_repo.upsert_field(
            db, inst_sched_1.id, "line_15",
            "Deductible part of self-employment tax",
            value_numeric=se_deductible_half, is_calculated=True,
        )
