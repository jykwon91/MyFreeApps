...
async def reassign_property(
    ...
    async with unit_of_work() as db:
        count = await db_admin_repo.bulk_update_property(
            db, body.vendor, body.filename_pattern, str(body.target_property_id),
        )
    logger.info(
        "ADMIN_DB reassign_property vendor=%s pattern=%s target=%s count=%d",
        body.vendor, body.filename_pattern, body.target_property_id, count,
    )
...
async def fix_sub_category(
    ...
    async with unit_of_work() as db:
        count = await db_admin_repo.bulk_update_sub_category(
            db, body.vendor, body.description_pattern, body.new_sub_category,
        )
    logger.info(
        "ADMIN_DB fix_sub_category vendor=%s pattern=%s sub_cat=%s count=%d",
        body.vendor, body.description_pattern, body.new_sub_category, count,
    )
    return BulkUpdateResponse(updated=count)
...
async def soft_delete_transactions(
    ...
    async with unit_of_work() as db:
        count = await db_admin_repo.bulk_soft_delete(
            db, body.vendor, body.category, body.source, body.description_pattern,
        )
    logger.info(
        "ADMIN_DB soft_delete vendor=%s category=%s source=%s count=%d",
        body.vendor, body.category, body.source, count,
    )
    return BulkUpdateResponse(updated=count)
...
async def re_extract_documents(
    ...
    async with unit_of_work() as db:
        count = await db_admin_repo.queue_documents_for_reextraction(
            db, [str(d) for d in body.document_ids],
        )
    logger.info("ADMIN_DB re_extract doc_count=%d", count)
    return BulkUpdateResponse(updated=count)
