...
## 2026-05-30 — Payment Review (rent attribution) — captured from operator session

Context: a Zelle payment from "Tushar Kapoor" ($1,600, memo "Rent") did not link to tenant "Prince Kapoor" (a relative paying — same surname, different first name; edit distance ~6 ≫ the ≤2 fuzzy threshold). Auto-match behaving as designed, but several gaps surfaced.

- [HIGH] ~~**Auto-match silently picks the FIRST tenant on duplicate names (latent wrong-attribution).**~~ **RESOLVED 2026-05-30** — `find_best_match` now detects ties (≥2 exact matches, or ≥2 fuzzy at the same smallest distance ≤2) and returns `(None, "ambiguous")`; `maybe_attribute_payment` routes that to the review queue (logged WARNING) instead of auto-confirming. Single exact/fuzzy matches unchanged. The richer "show the tied candidates with context" UX is the [DESIGN] item below. _(Original:_ `attribution_service.py::find_best_match` returned the first applicant whose `legal_name` matched and auto-confirmed; with two same-named `lease_signed` tenants it could attribute rent to the wrong one with NO review.)_

- [HIGH] ~~**"Remember for next time" is not implemented.**~~ **RESOLVED 2026-05-30** — added the `payer_alias` table (org_id, user_id, normalized_payer_name, nullable payer_handle, applicant_id FK CASCADE, source, timestamps; unique on (org, normalized name)) + migration `payeralias260530` (also widens `chk_txn_attribution_source` to admit `auto_alias`). `maybe_attribute_payment` gained a **Pass-0** alias lookup (before name matching) that auto-attributes (`attribution_source="auto_alias"`); `confirm_review` + `attribute_manually` upsert the alias. A future "Tushar Kapoor" payment now auto-attributes to Prince. Dangling-alias (deleted tenant) falls through to name matching. Frontend type union + AttributeTenantPicker display updated for the new source.

- [MED] ~~**No empty-state hint on the unmatched-rent path.**~~ **RESOLVED 2026-05-30** — `AttributionReviewItem` now has a tenant-picker sub-state machine mirroring the property picker (loading / errored / empty / ready). The empty state shows "No tenants with a signed lease yet — add one in Applicants." + a disabled Link, plus a visible "Loading tenants…" affordance on the pre-fetch tick.

- [HIGH] ~~**Picker may not surface promoted tenants.**~~ **RE-OPENED then RESOLVED 2026-05-30 (eve).** The earlier "VERIFIED — NOT A BUG" verdict was reached from a code read of `api/applicants.py:122-132` and was WRONG — the operator's live UI showed the attribute-to-tenant picker empty for a confirmed `lease_signed` tenant (Prince Kapoor, visible on the Tenants page). **Real root cause:** BOTH tenant pickers — `AttributeTenantPicker.tsx` (transaction panel) and `AttributionReviewItem.tsx` (Payment Review queue) — called `useGetApplicantsQuery({ stage: "lease_signed", limit: 200 })`, but `GET /applicants` caps `limit` at `Query(..., le=100)` (`api/applicants.py:125`). `limit=200` → **422 Unprocessable Entity** → RTK `data` undefined → `items` empty → the txn-panel picker rendered "No lease-signed tenants to link to" and the review-queue picker its error/empty state, **for every user regardless of stage**. (The "verified not a bug" check never exercised `limit=200`.) **Fix:** both pickers switched to `useGetTenantsQuery({ limit: 100 })` — the dedicated `/applicants/tenants` endpoint, same source as the Tenants page, within the cap. Regression tests added (`AttributeTenantPicker.test.tsx` pins the within-cap tenants query + renders options; `AttributionReviewItem.test.tsx` updated). The working callers (`LeaseImportDialog` `limit:100`, `ApplicantPicker` default) were always under the cap, which is why this stayed hidden.

- [DESIGN] ~~**Same-name disambiguation on the PAYER side (two different people, same name).**~~ **RESOLVED 2026-05-30 (PR3)** — the `payer_alias` unique key widened from `(org, normalized_payer_name)` to `(org, normalized_payer_name, payer_handle, applicant_id)` (migration `payerhandle260530`; `payer_handle` backfilled NULL→'' + NOT NULL so the key behaves identically on SQLite/Postgres). A name may now map to >1 tenant: extraction captures a stable sender handle (Zelle email/phone, Venmo @user, Cash App $tag — `base_prompt.py`) into the new nullable `transactions.payer_handle`, and `attribution_matcher.resolve_alias` resolves Pass-0 by handle-exact first, then name-level — a name confirmed to two tenants with no distinguishing handle returns `"ambiguous"` → review (no silent wrong-attribution), exactly the guard `find_best_match` applies to same-named tenants. `confirm_review`/`attribute_manually` seed the alias handle from the txn. `payer_handle` masked in `MBK_SENSITIVE_FIELDS`; not surfaced in the API read schema. _(The manual-link picker still shows only names; richer per-candidate context — address / lease term / contact last-4 — when ≥2 tenants tie remains a UX nicety, separate from this correctness fix.)_

Suggested grouping: ~~PR1 = empty-state hint + verify/fix picker query + refuse-to-guess-on-duplicate-names~~ **DONE 2026-05-30**; ~~PR2 = payer_alias learning~~ **DONE 2026-05-30**; ~~PR3 = stable sender-handle capture for same-name payers~~ **DONE 2026-05-30** (migration `payerhandle260530`; multi-row alias keyed on handle + ambiguity guard).

- [MED] **Auto-attribution candidate set includes ended tenants and contradicts its own docstring.** `attribution_service._get_lease_signed_applicants` (`attribution_service.py:44-59`) is documented "Fetch all **active** lease_signed applicants" but calls `applicant_repo.list_for_user(stage="lease_signed", limit=500)` with **no ended filter** — so `maybe_attribute_payment` can auto-attribute / propose a fresh rent payment to a moved-out tenant (`tenant_ended_at` set, or contract expired). The Tenants page and (now) both manual pickers use `list_tenants`, which excludes ended by default — so auto-match and manual-link now disagree on the candidate set. Decide the intended rule (almost certainly exclude ended for *new* payments) and align `_get_lease_signed_applicants` with `list_tenants`' ended filter, or document why ended must remain matchable. Separate from the 422 picker fix. _(`limit=500` vs the API's `le=100` is intentional here — it's a direct repo call, not the capped HTTP route.)_

- [LOW] **`attribution_service.py` crossed 500 LOC (492 → 522) in PR3.** Still under the 1000 hard limit and below the growth-guard's base-ref threshold for this PR, but the NEXT change that grows it will fail `check-file-size`. When that happens, extract the self-contained `_attribute_airbnb_payout` helper (~75 LOC) to a sibling module (mirrors PR2 splitting the pure matcher into `attribution_matcher.py`); the airbnb-payout tests patch `attribution_service.*` paths, so update those patch targets in the same move.

## 2026-08-30 — Test isolation: unregistered models break single-file backend runs

- [MED] **`tests/test_test_utils_routes.py` errors 17/22 when run alone: `no such table: pending_rent_receipts`.** `PendingRentReceipt` (`app/models/leases/pending_rent_receipt.py`) is not imported by any models package `__init__`, so at `conftest`'s `create_all` the table is absent from `Base.metadata` — but importing `app.main` later pulls it in via `services/leases/receipt_service.py`, so the `db` fixture's teardown loop over `metadata.sorted_tables` then tries to delete a table that was never created. Invisible in a full-suite run (some earlier module imports the model before `create_all`), which is why the suite is green; it bites `scripts/run-affected-tests.sh` whenever the admin domain is the only one selected. Fix by registering every model in the models package `__init__` (or an explicit `app/db/base.py` import block) so `create_all` and the teardown see the same set — a registry gap, not a rent-ledger issue. Reproduced on a branch with no rent-ledger code.
