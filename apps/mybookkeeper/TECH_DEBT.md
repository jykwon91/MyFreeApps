...
- [HIGH] **Auto-match silently picks the FIRST tenant on duplicate names (latent wrong-attribution).** `attribution_service.py::find_best_match` (`:78-96`) returns the first applicant whose `legal_name` matches and auto-confirms (`auto_exact`). With two same-named `lease_signed` tenants it can attribute rent to the wrong one with NO review. Fix: if ≥2 candidates tie at the best confidence (exact or fuzzy), do NOT auto-confirm — drop to the review queue flagged "multiple tenants named X" and show the tied candidates. Data-integrity issue; fix independent of the feature work.

- [HIGH] **"Remember for next time" is not implemented.** The Payment Review page + confirm toast promise it, but no payer→tenant alias is ever stored. `confirm_review`/`attribute_manually` (`attribution_service.py:323,428`) set `txn.applicant_id` on the one txn only; `find_best_match` matches `payer_name` vs `applicant.legal_name` only (exact, then Levenshtein ≤2) — no alias lookup. Build a `payer_alias` table (org_id, normalized payer_name, nullable payer_handle, applicant_id FK, source, created_at) + a matcher Pass-0 that resolves aliases to an exact tenant ID before name-fuzzy. Write the alias on confirm/manual-link. Realizes the UI promise AND makes future "Tushar Kapoor" payments auto-attribute.

- [MED] **No empty-state hint on the unmatched-rent path.** `AttributionReviewItem.tsx:206` renders the tenant-picker (`— pick a tenant —` dropdown + Link) only when `applicants.length > 0`; with zero `lease_signed` applicants it silently hides the picker, leaving only "Not them" — no explanation (the property path DOES show "No properties set up yet — add one in Settings", `:261`). Add an equivalent hint, e.g. "No tenants with a signed lease yet — add one in Tenants/Applicants."

- [MED/VERIFY] **Picker may not surface promoted tenants.** The picker uses `useGetApplicantsQuery({stage:"lease_signed"})` → `GET /applicants?stage=lease_signed` (`applicantsApi.ts:25`), but the Tenants page uses `getTenants` → `GET /applicants/tenants` (`:83`) — DIFFERENT endpoints. Operator has a `lease_signed` tenant (Prince Kapoor) yet reported only "Not them" on Payment Review. VERIFY whether `GET /applicants?stage=lease_signed` returns promoted tenants; if it doesn't, the picker is empty despite valid candidates (a real bug). If it DOES, the dropdown was likely just overlooked. Resolve before/with the empty-state fix.
...
Suggested grouping: PR1 = empty-state hint + verify/fix picker query + refuse-to-guess-on-duplicate-names; PR2 = payer_alias learning; PR3 (optional) = stable sender-handle capture for same-name payers.
