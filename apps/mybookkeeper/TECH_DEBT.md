...
- [DESIGN] **Same-name disambiguation (manual-link picker).** When ≥2 candidates share a name, show per-candidate context (property/unit address via lease→listing→property, active lease term, email/phone last-4) so the host picks correctly. Alias keyed on tenant ID (not name). Same-name on the PAYER side (two different people, same name) → key the alias on a stable sender handle (Zelle email/phone) — requires capturing it in extraction (`base_prompt.py` currently extracts `payer_name` only) + a `payer_handle` column; until then, mark a payer_name confirmed to >1 tenant as ambiguous → always review.

Suggested grouping: ~~PR1 = empty-state hint + verify/fix picker query + refuse-to-guess-on-duplicate-names~~ **DONE 2026-05-30**; ~~PR2 = payer_alias learning~~ **DONE 2026-05-30**; PR3 (optional, still open) = stable sender-handle capture for same-name payers (the [DESIGN] item — disambiguate two different people who share a name via a Zelle email/phone handle, using the reserved `payer_alias.payer_handle` column).
