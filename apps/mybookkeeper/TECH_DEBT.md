...
- [MED/VERIFY] ~~**Picker may not surface promoted tenants.**~~ **VERIFIED — NOT A BUG (2026-05-30).** `GET /applicants?stage=lease_signed` honors the stage filter and returns promoted tenants (`api/applicants.py:122-132`), so the picker renders valid candidates. Prince Kapoor's "only Not them" report is explained by the now-fixed missing empty-state (above) / the dropdown being overlooked, not an empty query. No query change needed.
