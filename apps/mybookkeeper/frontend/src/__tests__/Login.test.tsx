...
// ---------------------------------------------------------------------------
// Lockout (429) handling
// ---------------------------------------------------------------------------

describe("Login — lockout 429 handling", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("shows lockout message when login returns 429 with 'Too many' detail", async () => {
    const lockoutError = { status: 429, data: { detail: "Too many sign-in attempts. Try again later." } };
    vi.mocked(login).mockRejectedValue(lockoutError);
    const user = userEvent.setup();
    const { container } = renderLogin();
  ...
  it("disables submit button and shows countdown after lockout 429", async () => {
    const lockoutError = { status: 429, data: { detail: "Too many sign-in attempts. Try again later." } };
    vi.mocked(login).mockRejectedValue(lockoutError);
    const user = userEvent.setup();
    const { container } = renderLogin();
    ...
    await waitFor(() => {
      const button = screen.getByRole("button", { name: /Try again in/i });
      expect(button).toBeDisabled();
    });
  });

  it("shows generic error (not lockout message) for non-429 failures", async () => {
    vi.mocked(login).mockRejectedValue(new Error("Unauthorized"));
    const user = userEvent.setup();
    const { container } = renderLogin();
    ...
    await screen.findByText("Invalid email or password");
    expect(screen.queryByText(/Too many sign-in attempts/i)).not.toBeInTheDocument();
  });
});
...
describe("Login — unverified user", () => {
  const unverifiedError = { data: { detail: "LOGIN_USER_NOT_VERIFIED" } };

  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("shows verification prompt when backend returns LOGIN_USER_NOT_VERIFIED", async () => {
    vi.mocked(login).mockRejectedValue(unverifiedError);
    const user = userEvent.setup();
    const { container } = renderLogin();
  ...
  it("shows resend verification email button when unverified", async () => {
    vi.mocked(login).mockRejectedValue(unverifiedError);
    const user = userEvent.setup();
    const { container } = renderLogin();
  ...
  it("does not show generic error message for unverified response", async () => {
    vi.mocked(login).mockRejectedValue(unverifiedError);
    const user = userEvent.setup();
    const { container } = renderLogin();
  ...
  it("calls /auth/request-verify-token when resend button is clicked", async () => {
    vi.mocked(login).mockRejectedValue(unverifiedError);
    ...
    await waitFor(() => {
      expect(api.post).toHaveBeenCalledWith("/auth/request-verify-token", { email: "user@example.com" });
    });
  });
  ...
  it("shows error message when resend fails", async () => {
    vi.mocked(login).mockRejectedValue(unverifiedError);
    vi.mocked(api.post).mockRejectedValue(new Error("SMTP down"));
    const user = userEvent.setup();
    const { container } = renderLogin();
    ...
    await screen.findByText(/failed to resend/i);
  });
});