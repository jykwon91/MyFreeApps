...
describe("Dashboard", () => {
  ...
  it("renders monthly average card with correct averages", () => {
    renderWithProviders(<Dashboard />);

    expect(screen.getByText("Monthly Average")).toBeInTheDocument();
    expect(screen.getByText("across 2 months")).toBeInTheDocument();
    // Revenue avg = (2500 + 2500) / 2 = 2500
    // Expenses avg = (1000 + 1000) / 2 = 1000
    // Profit avg = (1500 + 1500) / 2 = 1500
    const card = screen.getByText("Monthly Average").closest("div")!.parentElement!;
  ...
  it("hides monthly average card when no month data", () => {
    vi.mocked(useGetSummaryQuery).mockReturnValue({
