...
class UserManager(UUIDIDMixin, BaseUserManager[User, uuid.UUID]):
    ...
    async def authenticate(
        ...
        unknown email → log domain-only LOGIN_FAILURE; locked → reject before
        password check + emit LOGIN_BLOCKED_LOCKED; auto-reset stale counter;
        on bad password → increment + maybe lock; on success → clear + log.
        """
