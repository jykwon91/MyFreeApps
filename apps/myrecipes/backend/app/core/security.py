"""MyRecipes encryption helpers.

Two key families derived from a single ``encryption_key`` master via HKDF:

1. **PII** — ``salt=b"myrecipes-v1"``,
   ``info=b"myrecipes-pii-encryption"``. Used by the
...
_PII_SALT = b"myrecipes-v1"
_PII_INFO = b"myrecipes-pii-encryption"
