...
#   python -m platform_shared.infra.render --app myrecipes
#
...
# All COPY paths are relative to MyFreeApps/, not apps/myrecipes/.
#
...
COPY apps/myrecipes/frontend/package.json ./apps/myrecipes/frontend/
RUN npm ci --ignore-scripts
COPY packages/shared-frontend ./packages/shared-frontend
COPY apps/myrecipes/frontend ./apps/myrecipes/frontend
...
RUN npm run build --workspace=apps/myrecipes/frontend

FROM caddy:2-alpine
COPY --from=frontend-build /repo/apps/myrecipes/frontend/dist /srv/frontend
COPY apps/myrecipes/docker/Caddyfile.docker /etc/caddy/Caddyfile
