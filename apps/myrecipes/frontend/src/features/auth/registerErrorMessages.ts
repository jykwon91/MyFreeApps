 ...
 *     a nested object whose ``reason`` is already human-readable (this is
 *     where the HIBP breach message and the length rule surface).
 *
 * Without this mapping the raw code (or, worse, axios's
 * ``"Request failed with status code 400"``) is what the user sees.
 * Mirrors apps/myjobhunter/frontend/src/features/auth/registerErrorMessages.ts.
 */
